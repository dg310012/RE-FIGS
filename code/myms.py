...
def GenerateDecoyLibrary(SpectraLibrary, distance):
    ...
    for charge in charges:
        ...
        while i < len(key_premz) and j < len(key_premz):
            if i % 1000 == 0:
                print('\r' + str(i)+"/"+str(len(LibraryDividedByCharge)),
                      end='', flush=True)
            ...
            if abs(j_premz-i_premz) >= distance:
                ...
                DecoyLibraryDividedByCharge[decoy_i_key]['Spectrum'] = DecoyLibraryDividedByCharge[
                    decoy_i_key]['Spectrum'][DecoyLibraryDividedByCharge[decoy_i_key]['Spectrum'][:, 0] > 0]
                DecoyLibraryDividedByCharge[decoy_j_key]['Spectrum'] = DecoyLibraryDividedByCharge[
                    decoy_j_key]['Spectrum'][DecoyLibraryDividedByCharge[decoy_j_key]['Spectrum'][:, 0] > 0]
                ###################
        ...
        if len(LibraryDividedByCharge) != len(DecoyLibraryDividedByCharge):
            ...
            )) - (set(swapped_keys) | set(swapped_keys.inv))
                ...
                for swapped_key in sorted(swapped_keys):
                    ...
                    if abs(unswapped_premz - SpectraLibrary[swapped_key]['PrecursorMZ']) >= distance and \
                       abs(unswapped_premz - SpectraLibrary[swapped_key2]['PrecursorMZ']) >= distance:
                        DecoyLibraryDividedByCharge[(
                            'DECOY-'+unswapped_key[0], unswapped_key[1])] = SpectraLibrary[unswapped_key].copy()
                        DecoyLibraryDividedByCharge[(
                            'DECOY-'+unswapped_key[0], unswapped_key[1])]['Spectrum'] = SpectraLibrary[swapped_key2]['Spectrum'].copy()

                        DecoyLibraryDividedByCharge[(
                            'DECOY-'+swapped_key[0], swapped_key[1])]['Spectrum'] = SpectraLibrary[unswapped_key]['Spectrum'].copy()

                        DecoyLibraryDividedByCharge[('DECOY-'+swapped_key[0], swapped_key[1])]['Spectrum'][:, 0] += (
                            SpectraLibrary[swapped_key]['PrecursorMZ']-unswapped_premz)
                        DecoyLibraryDividedByCharge[('DECOY-'+unswapped_key[0], unswapped_key[1])]['Spectrum'][:, 0] += (
                            SpectraLibrary[unswapped_key]['PrecursorMZ']-SpectraLibrary[swapped_key2]['PrecursorMZ'])
...
def cal_nnls(LibIntensity, MS2Intensity, penalty):
    ...
    LibraryCoeffs = sparse_nnls.lsqnonneg(
        LibraryVector, MS2Intensity, {'show_progress': False})
...
#                                 if spectrumRT-5 < float(spectrum['PrecursorRT']) < spectrumRT+5]
    ...
    CandidateLibrarySpectra = [RefSpectraLibrary[key]
                               ['Spectrum'] for key in PrecursorCandidates]

    peaknum = len(DIASpectrum)
    while True:
        MergedSpectrumCoordIndices = np.searchsorted(
            DIASpectrum[:, 0] + tol * DIASpectrum[:, 0], DIASpectrum[:, 0])
        MergedSpectrumCoords = DIASpectrum[np.unique(
            MergedSpectrumCoordIndices), 0]
        ...
        DIASpectrum = np.array(
            (MergedSpectrumCoords, MergedSpectrumIntensities)).transpose()
    ...
    LocateReferenceCoordsInDIA = [np.searchsorted(
        CriticalMZs, lib[:, 0]) for lib in CandidateLibrarySpectra]
    ...
    ReferencePeaksInDIA = [i for i in range(len(PrecursorCandidates)) if
                           len([a for a in TopTenPeaksCoordsInDIA[i] if a %
                               2 == 1]) >= 5
                           ]

    IdentPrecursorsLocations = [LocateReferenceCoordsInDIA[i]
                                for i in ReferencePeaksInDIA]

    IdentPrecursorSpectra = [CandidateLibrarySpectra[i]
                             for i in ReferencePeaksInDIA]
    ...
    for i in range(len(IdentPrecursors)):
        coeff = 0
        unique_mz_ids_of_thislib = unique_mz_ids[np.where(
            unique_mz_lib_ids == i)]
        if len(unique_mz_ids_of_thislib) > 0:
            ...
            FeaturedPeakIntensitiesInLib = FeaturedPeakIntensitiesInLib.reshape(
                1, -1)[0]

            if Top10First:
                TargetDecoyLibrary = SpectraLibrary.copy()
                MinIntensityDic = {k: np.sort(v['Spectrum'][:, 1])[-min(10, len(v['Spectrum']))] for k, v in
                                   TargetDecoyLibrary.items()}
                CandidateIndex = np.where(
                    FeaturedPeakIntensitiesInLib >= MinIntensityDic[IdentPrecursors[i]])
            ...
            coeff = cal_nnls(list(FeaturedPeakIntensitiesInLib), list(
                FeaturedPeakIntensitiesInDIA), Penalties[i])
        else:
            flag = True
        output.append(
    ...
    if flag and level < 50:
        LibraryMatrix = sparse.coo_matrix(
            (MatrixIntensities, (RowIndices, ColumnIndices)))
        ...
        DIASpectrum[UniqueRowIndices,
                    1] = DIASpectrum[UniqueRowIndices, 1] - FittingIntensities
        ...
        output.extend(deconvolute(SpectrumInfo, CopyLibrary,
                      tol, Top10First, level + 1))
    ...
    if level == 1:
        ...
        Correlation = pd.DataFrame(
            [FittingIntensities, DIASpectrumIntensities]).T.corr()[0][1]
        for i in output:
